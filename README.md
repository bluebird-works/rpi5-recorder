...
|---|---|---|
| `rpicam-apps` | `rpicam-vid` — захоплення H.264; `rpicam-raw` — сирий Bayer (режим C) | усі режими |
...
5. **«Старт»** — запускає запис у поточних налаштуваннях (див. «Режим зйомки» нижче; за замовчуванням — `WIDTH/HEIGHT/FPS/BITRATE` з env).
...
### Режим зйомки (роздільність і FoV)

Секція **«Режим зйомки»** під кнопками — три контроли, які змінюють параметри **наступного** запису (під час активного запису вони заблоковані: камера вже відкрита з іншими параметрами, перезапуск на льоту рвав би файл).

**Режим сенсора (FoV).** Список не захардкоджений — він читається з реального заліза: `recording_engine._parse_modes()` парсить блок `Modes:` з `rpicam-vid --list-cameras`, включно з crop-ректом кожного режиму. Crop-рект і є полем зору: якщо він менший за повний сенсор, кадр вужчий. Для Camera Module 3 (IMX708) виходить:

| Режим | Стеля fps | FoV |
|---|---|---|
| 1536×864 | 120 | кроп 0.67× (3072×1728 з 4608×2592) |
| 2304×1296 | 56 | повний |
| 4608×2592 | 14 | повний |

На іншому сенсорі (IMX219, IMX477, HQ) список буде свій — нічого міняти в коді не треба. Обраний режим іде в `rpicam-vid --mode W:H`, і в `rpicam-raw`, і в `rpicam-jpeg` для «перевірити кадр» — щоб знімок показував рівно той кадр, що піде в запис. `auto` = хай libcamera обирає сама (стара поведінка).

**Роздільність запису** — окремо від режиму, і це головне, заради чого воно розділене: режим `2304:1296` (повний FoV) з виводом `1920×1080` дає **повний кадр у 1080p**, який ще тягне апаратний енкодер. Список фільтрується під обраний режим: не більше за нього і те саме співвідношення сторін (±2%), плюс завжди рідна роздільність самого режиму. Варіанти ширші за 1920 підписані «SW, гріється» — це стеля HW-енкодера.

**FPS** — обмежений стелею обраного режиму **і роздільності**; сервер це перевіряє незалежно від UI (`120` на режимі `2304:1296` відхиляється з поясненням). Стеля видна прямо в списку роздільностей (`1280×720 · ≤60 fps`).

**Стеля fps на Pi 4.** Режими сенсора обіцяють до 120 fps, але Pi 4 стільки не тягне — впирається вже ISP, а не енкодер. Заміряно на IMX708 (реальні кадри / час запису):

| Запис | Реально |
|---|---|
| 1920×1080@30, 1536×864@30, 2304×1296@30 (SW) | 30 — чисто |
| 1280×720@60 | 60 — чисто |
| 1536×864@60 | ~55 — дропи |
| 1280×720@120 | ~67 |
| 1920×1080@56 (режим 2304:1296) | ~35 |
| 1536×864@120 | не стартує (HW H.264 level 4.2) |

Тому на Pi 4 (визначається за `bcm2835-codec-encode`) дозволено **до 60 fps для кадру ≤1280×720 і до 30 fps для більшого**. Перевизначається `PI4_FPS_MAX` / `PI4_FPS_MAX_SMALL`. На Pi 5 і USB-камері стелі нема. Чому це важливо: mp4 маркується **заявленим** fps, скільки б кадрів камера не віддала — недобір не видно ні в статусі, ні в ffprobe, тільки при перегляді (відео прискорене).

**Перевірка після запису.** Після кожного стопу рекордер у фоні рахує кадри у файлі (`ffprobe -count_packets`) і ділить на реальний час запису. Результат лягає поруч у `rec_….json`, а в таблиці записів замість «готово» з'являється **⚠ 55 з 60 fps**, якщо вийшло менше `FPS_CHECK_RATIO` (0.95) від заявленого. Записи коротші за 5 с і сегментований режим (`SEGMENT_SEC>0`) не перевіряються — там час на окремий файл невідомий або старт пайплайна дає хибну тривогу.

Вибір зберігається в `~/recordings/.settings.json` і переживає ребут та power-cut — на старті сервісу він перекриває значення з env у systemd-юніті. Для USB-камери (`CAMERA_SRC=usb`) селектор режиму ховається: у UVC поняття сенсорного режиму й FoV не існує, лишається роздільність і fps.

Заодно полагоджено давню тиху пастку: при ширині >1920 апаратний енкодер мовчки різав кадр (`asked for 2304x1296, got: 1920x1296`). Тепер `ENCODER=hardware` вище цієї межі автоматично падає в софтверний шлях замість кропу.

### Таблиця записів

Під статусом — таблиця всіх записів: назва, розмір, дата/час, статус (`Saved` / `Recording` для активного файлу), і дії. **Download** качає файл прямо через браузер. **Delete** — з модалкою підтвердження. Активний файл видалити не можна (кнопки нема). Захист від path traversal: ім'я валідується шаблоном `rec_YYYYMMDD_HHMMSS.(mp4|raw)`, шлях перевіряється на вихід за межі теки записів.

Забрати відео можна і напряму, як в інших режимах:
```bash
scp pi@10.42.0.1:~/recordings/*.mp4 ./
```

### Перейменування точки доступу
...
Веб-сервіс сам прав на `nmcli` не має (крутиться під `${REC_USER}`, тільки `CAP_NET_BIND_SERVICE`). Він лише пише бажаний конфіг у spool `/var/lib/rpi5-web/spool/ap-config.json` (pi-writable підтека); окремий root-owned systemd path-юніт (`rpi5-ap-apply.path`) бачить зміну й запускає `/usr/local/sbin/rpi5-ap-apply` (root-only, поза pi-writable теками), який **перевіряє все заново** — SSID/пароль читаються одним `O_NOFOLLOW`-open (без symlink-TOCTOU) і валідуються незалежно від веб-шару — і застосовує через `nmcli con modify`. Ніякого sudo/setuid/polkit — привілеї не течуть у веб-процес. Поточний стан AP root пише в `/var/lib/rpi5-web/ap-current.json` (root-only тека, pi лише читає) — навмисно окремо від spool, щоб pi не міг підмінити його симлінком. Два незалежні security-рев'ю (Codex + Opus) прогнали саме цю межу; знайдені дірки (symlink-swap current-файлу; root-executed скрипт у pi-owned теці) закриті.

### Raw-data режим

Чекбокс **«raw-data»** біля «Старт» перемикає запис у сирий Bayer (`rpicam-raw`) замість H.264. Файл `rec_….raw` — headerless потік прямо з сенсора, поряд пишеться `rec_….json` з метаданими (сенсор, роздільність, bit-depth, Bayer-порядок), без якого потік неможливо прочитати. Delete зносить обидва файли.

⚠️ **Raw їсть диск на два порядки швидше H.264.** Тому: дефолтна частота `RAW_FPS=10` (вище `rpicam-raw` дропає кадри), і **watchdog вільного місця** — нижче `FREE_MB_MIN` (500 МБ) запис зупиняється сам і зберігає вже зняте (не ротує його по колу, як H.264). У статусі тоді видно «зупинено: мало місця». На 32-ГБ карті raw 1080p — це одиниці хвилин, плануй місце.
...
|---|---|---|
| `WIDTH` / `HEIGHT` | `1920` / `1080` | роздільність запису. **Вище 1920 по ширині HW-енкодер не вміє** — там код сам переходить на софтверний шлях |
...
| `SENSOR_MODE` | `` (auto) | режим сенсора `W:H` для `rpicam --mode` — задає FoV і стелю fps окремо від `WIDTH`/`HEIGHT`. У режимі C зручніше міняти з веб-UI (вибір зберігається у `~/recordings/.settings.json` і перекриває цю змінну) |
...
| `FREE_MB_MIN` | `500` | нижче цього вільного місця — примусова ротація (режим B) / авто-стоп raw (режим C) |
| `RAW_FPS` | `10` | частота raw-запису; вище `rpicam-raw` дропає кадри (тільки режим C) |
