...
Дефолтний пароль AP — `12345678`, свідомо простий: пристрій польовий, оператор набирає пароль на телефоні, мережа закрита й без виходу в інтернет. Хто працює на локації з кількома рекордерами — міняє SSID/пароль через веб-інтерфейс або задає `AP_PASSWORD=...` на деплої. Поточний пароль дублюється у `~/rpi5-web/.ap_password` (chmod 600, власник — юзер рекордера), щоб не загубився у scrollback:
...
| `AP_PASSWORD` | `12345678` | пароль WPA2 точки доступу. Змінюється через веб-UI або на деплої (тільки режим C) |
| `AP_COUNTRY` | `UA` | код країни для wireless-регдомену, потрібен щоб AP підняв 2.4 ГГц канал (тільки режим C) |
